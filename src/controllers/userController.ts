...
export default class UserController {
  ...
  async getUserList(c: Context) {
    ...
    const apiConfig: ApiConfig<DataTotal<UserRole>> = new ApiConfig<DataTotal<UserRole>>();
  ...
  async getRandHeadImg(c: Context) {
    ...
    const files = fs.readdirSync(path.join(__dirname, '../../static/img/updateImg'));
  ...
  async addUser(c: Context) {
    ...
    if (checkObj(params, ["uname", "username", "signature", "head_img"])) {
      ...
      const apiConfig: ApiConfig<string> = new ApiConfig<string>();
      // 返回一个失败的 ApiConfig 对象，包含提示信息
      result = apiConfig.fail("请检查内容是否填写完整");
    }

    //获取当前请求来源的ip
    params.create_ip = c.req.header('x-forwarded-for') || c.req.header('cf-connecting-ip') || c.req.header('x-real-ip') || "::1"
  ...
  async updateUser(c: Context) {
    ...
    const apiConfig: ApiConfig<string> = new ApiConfig<string>();
    //检查参数是否包含 id
    if (checkObj(params, ["uid"], ["uname", "password", "power", "whether_use", "signature", "head_img"])) {
      // 返回一个失败的 ApiConfig 对象，包含提示信息
  ...
  async uploadHeadImg(c: Context) {
    ...
    const uploadPath = path.join(__dirname, '../../static/img/uploadHead', filename);
